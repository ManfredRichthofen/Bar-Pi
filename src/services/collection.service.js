...
class CollectionService {
	...
	getCollectionsByUser(id) {
		...
		return axios
			.get(API_PATH, { params })
			.then((response) =>
				response.data.map((x) => this.afterCollectionLoad(x)),
			);
	}
	...
	removeRecipeFromCollection(collectionId, recipeId) {
		return axios.delete(
			API_PATH + String(collectionId) + "/" + String(recipeId),
		);
	}

	getCollectionRecipes(collectionId) {
		return axios
			.get(API_PATH + String(collectionId) + "/recipes")
			.then((response) =>
				response.data.map((x) => RecipeService.afterRecipeLoad(x)),
			);
	}
