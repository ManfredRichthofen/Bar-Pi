...
const FilterPanel = React.memo(({
  ...
  const allFilters = ['automatic', 'manual', 'available', 'alcoholic', 'nonAlcoholic', 'favorited', 'fabricable'];
  
  // Mode-specific filters
  const simpleFilters = ['automatic', 'manual', 'available'];
  const advancedFilters = ['alcoholic', 'nonAlcoholic', 'favorited', 'fabricable'];
  
  const activeFilters = variant === 'simple' ? simpleFilters : advancedFilters;
  ...
  const getButtonClassName = (isActive) => {
    const baseClasses = "h-10 rounded-lg transition-all duration-200 hover:scale-105";
  ...
  const FilterButton = ({ filter, isActive }) => (
    ...
    >
      <span className="flex items-center gap-1.5">
        {variant === 'simple' && (
          <span className={`w-2 h-2 rounded-full ${isActive ? 'bg-primary' : 'bg-muted-foreground'}`}></span>
        )}
        {filterLabels[filter]}
      </span>
    </Button>
      ...
      <div className={`flex flex-col gap-3 ${className}`}>
        <div className="flex flex-wrap gap-2">
          {activeFilters.map((filter) => (
            <FilterButton key={filter} filter={filter} isActive={filters[filter]} />
          ))}
        </div>
        {hasActiveFilters && (
          <Button
  ...
  return (
    <div className={`bg-gradient-to-br from-card to-card/50 border-2 border-primary/20 rounded-xl px-4 sm:px-6 py-6 space-y-4 shadow-lg backdrop-blur-sm animate-in slide-in-from-top-2 duration-300 ${className}`}>
